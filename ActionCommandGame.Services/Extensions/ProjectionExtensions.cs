...
public static class ProjectionExtensions
{
    public static IQueryable<PlayerResult> ProjectToResult(this IQueryable<Player> query)
    {
        return query.Select(p => new PlayerResult
        {
            ...
            CurrentAttackName =
                p.CurrentAttackPlayerItemId.HasValue ? p.CurrentAttackPlayerItem.Item.Name : string.Empty,
            CurrentDefenseId = p.CurrentDefensePlayerItemId,
            CurrentDefenseName = p.CurrentDefensePlayerItemId.HasValue
                ? p.CurrentDefensePlayerItem.Item.Name
                : string.Empty,
            ...
            CurrentFuelActionCooldownSeconds = p.CurrentFuelPlayerItemId.HasValue
                ? p.CurrentFuelPlayerItem.Item.ActionCooldownSeconds
                : 0,
    ...
    public static IQueryable<NegativeGameEventResult> ProjectToResult(this IQueryable<NegativeGameEvent> query)
    {
        return query.Select(nge => new NegativeGameEventResult
        {
            ...
            DefenseWithArmorDescription = nge.DefenseWithArmorDescription,
            DefenseWithoutArmorDescription = nge.DefenseWithoutArmorDescription,
