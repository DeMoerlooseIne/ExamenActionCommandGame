...
public class GameService : IGameService
{
    ...
    public async Task<ServiceResult<GameResult>> PerformActionAsync(int playerId, string authenticatedUserId)
    {
        ...
        if (player is null)
            return new ServiceResult<GameResult>
            {
                Messages = new List<ServiceMessage>
                    { new() { Code = "NotFound", Message = $"{nameof(Player)} was not found" } }
            };
        ...
        if (player.LastActionExecutedDateTime.HasValue)
        {
            ...
            if (elapsedSeconds < cooldownSeconds)
            {
                ...
                return new ServiceResult<GameResult>
                {
                    ...
                    Messages = new List<ServiceMessage> { new() { Code = "Cooldown", Message = waitText } }
                };
            }
        }
        ...
        var positiveGameEvent =
            await _positiveGameEventService.GetRandomPositiveGameEvent(hasAttackItem, authenticatedUserId);
        ...
        if (oldLevel < newLevel)
            levelMessages = new List<ServiceMessage>
                { new() { Code = "LevelUp", Message = $"Congratulations! You're at level {newLevel} now!" } };
        ...
        if (positiveGameEvent.Data.Money > 0) attackMessages.AddRange(ConsumeAttack(player, authenticatedUserId));
        ...
        if (negativeGameEvent.Data is not null)
        {
            ...
            if (player.CurrentDefensePlayerItem != null)
            {
                ...
                { Code = "DefenseWithGear", Message = negativeGameEvent.Data.DefenseWithArmorDescription });
                defenseMessages.AddRange(
                    ConsumeDefense(player, authenticatedUserId, negativeGameEvent.Data.DefenseLoss));
            }
            else
            {
                negativeGameEventMessages.Add(new ServiceMessage
                { Code = "DefenseWithoutGear", Message = negativeGameEvent.Data.DefenseWithoutArmorDescription });
                ...
                defenseMessages.AddRange(ConsumeAttack(player, authenticatedUserId,
                    negativeGameEvent.Data.DefenseLoss));
            }
        }
    ...
    public async Task<ServiceResult<BuyResult>> BuyAsync(int playerId, int itemId, string authenticatedUserId)
    {
        ...
        if (player == null) return new ServiceResult<BuyResult>().PlayerNotFound();

        var item = await _database.Items.SingleOrDefaultAsync(i => i.Id == itemId);
        if (item == null) return new ServiceResult<BuyResult>().ItemNotFound();

        if (item.Price > player.Money) return new ServiceResult<BuyResult>().NotEnoughMoney();
            ...
            if (player.CurrentFuelPlayerItem.RemainingFuel <= 0)
            {
                ...
                if (newFuelItem != null)
                {
                    ...
                    return new List<ServiceMessage>
                    {
                        ...
                        {
                            Code = "ReloadedFuel",
                            Message = $"You have to get your enerylevels up! Get a new {newFuelItem.Item.Name}!"
                        }
                    };
                }
                ...
                return new List<ServiceMessage>
                {
                    ...
                    {
                        Code = "NoFuel",
                        Message = "You can't go on like this. You need to get your energylevels up! RIGHT NOW!",
                ...
                if (newAttackItem != null)
                {
                    ...
                    return new List<ServiceMessage>
                    {
                        new()
                        {
                            Code = "ReloadedAttack",
                            Message =
                                $"You just broke {oldAttackItemName}. No worries, you swiftly wield a new {newAttackItem.Item.Name}. Yeah!"
                        }
                    };
                }
            ...
            if (player.CurrentDefensePlayerItem.RemainingDefense <= 0)
            {
                ...
                if (newDefenseItem != null)
                {
                    ...
                    return new List<ServiceMessage>
                    {
                        new()
                        {
                            ...
                            Message =
                                $"Your {oldDefenseItemName} is starting to smell. No worries, you swiftly put on a freshly washed {newDefenseItem.Item.Name}. Yeah!"
                        }
                    };
                }
                ...
                return new List<ServiceMessage>
                {
                    new()
                    {
                        Code = "NoAttack",
                        Message =
                            $"You just lost {oldDefenseItemName}. Don't continue without protection. You need a new Armor!",
                        MessagePriority = MessagePriority.Warning
                    }
                };
            }
    ...
    private IList<ServiceMessage> GetWarningMessages(Player player)
    {
        ...
        {
            var infoText =
                "Traveling space without fuel is hard. You need a long time to recover. Consider buying fuel in the shop.";
            serviceMessages.Add(new ServiceMessage
            { Code = "NoFood", Message = infoText, MessagePriority = MessagePriority.Warning });
        }

        if (player.CurrentAttackPlayerItem == null)
        {
            var infoText =
                "Traveling space without weapons is hard. You lost extra fuel. Consider buying weapons from the shop.";
            serviceMessages.Add(new ServiceMessage
            { Code = "NoTools", Message = infoText, MessagePriority = MessagePriority.Warning });
        }

        if (player.CurrentDefensePlayerItem == null)
        {
            var infoText =
                "Traveling space without armor is hard. You lost extra fuel. Consider buying armor from the shop.";
            serviceMessages.Add(new ServiceMessage
            { Code = "NoGear", Message = infoText, MessagePriority = MessagePriority.Warning });
        }
