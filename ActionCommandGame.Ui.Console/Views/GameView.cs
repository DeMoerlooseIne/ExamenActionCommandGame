...
internal class GameView : IView
{
    ...
    {
        ConsoleWriter.WriteText($"Play your game. Try typing \"help\" or \"{_settings.ActionCommand}\"",
            ConsoleColor.Yellow);
        ...
        while (true)
        {
            ...
            if (CheckCommand(command, new[] { "shop", "store" })) await _navigationManager.NavigateTo<ShopView>();
            ...
            if (CheckCommand(command, new[] { "buy", "purchase", "get" }))
            {
                ...
                if (!itemId.HasValue)
                {
                    ConsoleWriter.WriteText(
                        "I have no idea what you mean. I have tagged every item with a number. Please give me that number.",
                        ConsoleColor.Red);
                    continue;
                }
            ...
            if (CheckCommand(command,
                    new[] { "bal", "balance", "money", "xp", "level", "statistics", "stats", "stat", "info" }))
                await ShowStats(currentPlayerId);
    ...
    public async Task ShowStats(int playerId)
    {
        ...
        ConsoleWriter.WriteText($"{player.GetLevel()} ({player.Experience}/{player.GetExperienceForNextLevel()})  ",
            null, false);
